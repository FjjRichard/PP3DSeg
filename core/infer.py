...
def get_reverse_list(ori_shape, transforms):
    ...
    Returns:
        list: List of tuple, there are two format:
            ('resize3D', (d,h, w)) The image shape before resize,
            ('padding', (d,h, w)) The image shape before padding.
    """
    reverse_list = []
    d,h, w = ori_shape[0], ori_shape[1], ori_shape[2]
    for op in transforms:
        if op.__class__.__name__ in ['Resize3D']:
            reverse_list.append(('resize3D', (d,h, w)))
    ...
    for item in reverse_list[::-1]:
        if item[0] == 'resize3D':
            ...
            if paddle.get_device() == 'cpu':
                ...
                pred = F.interpolate(pred, (d,h, w), mode=mode,data_format='NCDHW')
...
def inference(model,
    ...
    if ori_shape is not None:
        pred = reverse_transform(logit, ori_shape, transforms, mode='trilinear')
        pred = paddle.argmax(pred, axis=1, keepdim=False, dtype='int32')
        return pred
    else:
        pred = paddle.argmax(logit, axis=1, keepdim=False, dtype='int32')
