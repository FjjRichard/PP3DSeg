...
class _MaskingLossWrapper(nn.Layer):
    """
...
class SkipLastTargetChannelWrapper(nn.Layer):
    """
...
def flatten(tensor):
    ...
    transposed = paddle.transpose(transposed, perm=list(axis_order))
...
class _AbstractDiceLoss(nn.Layer):
    """
    ...
    def __init__(self, weight=None, normalization='sigmoid'):
        ...
        if normalization == 'sigmoid':
            ...
            self.normalization = nn.Softmax(axis=1)
        else:
    ...
    def forward(self, input, target):
        ...
        return 1. - paddle.mean(per_channel_dice)
...
def get_loss_criterion(config):
    ...
    if weight is not None:
        ...
        weight = paddle.tensor(weight).to(config['device'])

    pos_weight = loss_config.pop('pos_weight', None)
    if pos_weight is not None:
        # convert to cuda tensor if necessary
        pos_weight = paddle.tensor(pos_weight).to(config['device'])
